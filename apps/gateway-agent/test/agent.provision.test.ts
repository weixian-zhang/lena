...
import { createPiAgent } from "@lena/agent/agent";
import { SYSTEM_PROMPT } from "@lena/agent/system-prompt";
import { isMutatingCommand } from "@lena/agent/tools/bash";
import { tools } from "@lena/agent/tools/index";
...
// throwaway resource lifecycle. Run it locally with apps/gateway-agent/.env populated; it
// hits the network and SKIPS (rather than fails) when the model endpoint is unset.

// index.ts loads .env only when run as the app; load it here too so the config +
// credentials resolve under the test runner.
try {
